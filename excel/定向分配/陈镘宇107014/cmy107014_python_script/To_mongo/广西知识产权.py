...
#使用xlrd打开工作本（excel文件）
book = xlrd.open_workbook(r'D:\白名单\定向分配资源整理\excel\定向分配\陈镘宇107014\名单\广西知识产权.xls')
...
for i in range(sheet.nrows):
    if i != 0:
        ...
        phone= str(sheet.row_values(i)[6])
        tal = sheet.row_values(i)[7]

        item["经营范围"] = None
        item["地址"] = sheet.row_values(i)[4]
        item["法定代表人"] = sheet.row_values(i)[2]
        item["成立日期"] = sheet.row_values(i)[3]
