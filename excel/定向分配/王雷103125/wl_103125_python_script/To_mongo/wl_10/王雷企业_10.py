...
@author     ：  yangwenlong
@file       :   ztl
@intro      :   王雷103125存入monmgo
@creatime   :   2019/9/26
'''
import xlrd
import hashlib
import datetime
from wl_103125_python_script.common import Enterprise_db,get_log

log = get_log()


#使用xlrd打开工作本（excel文件）
book = xlrd.open_workbook(r'D:\白名单\定向分配资源整理\excel\定向分配\王雷103125\10\王雷企业_10.xlsx')
...
def save(item):
    ...
    if item["电话"]:
        if item["成立日期"]:
            item["成立日期"] = str(datetime.datetime(*xlrd.xldate_as_tuple(sheet.row_values(i)[4], 0)))
            print(item["成立日期"])
